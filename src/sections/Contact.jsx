      ...
      <AnimatePresence>
        {error && (
          <motion.p
            ...
            style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.58rem", color:"#f87171", letterSpacing:"0.08em", display:"flex", alignItems:"center", gap:4, margin:0 }}
          >
...
export default function Contact() {
  ...
  // status: "" | "transmitting" | "success" | "error"
  ...
  // ── KEY FIX: always call preventDefault, never let form navigate ──────────
  const handleSubmit = (e) => {
    e.preventDefault();       // ← stops page reload / navigation
    e.stopPropagation();      // ← extra safety
    if (!validate()) return;
    setStatus("transmitting");
  };

  // Called after transmission animation finishes
  ...
  const inputStyle = (field) => ({
    ...BASE_INPUT,
    border: `1px solid ${errors[field] ? "rgba(248,113,113,0.6)" : "rgba(255,255,255,0.08)"}`,
    boxShadow: focusedField === field
      ? `0 0 0 2px ${errors[field] ? "rgba(248,113,113,0.3)" : "rgba(167,139,250,0.25)"}`
      : "none",
  ...
  return (
    <>
      <style>{`
        ...
        .contact-root {
          background:#050810; color:#fff; min-height:100vh;
        ...
        .contact-scan {
          ...
          animation:ct-scan 10s linear infinite; pointer-events:none; z-index:2;
        }
        @keyframes ct-scan { from{top:0} to{top:100%} }

        .hud-label {
          font-family:'JetBrains Mono',monospace; font-size:.55rem;
          letter-spacing:.18em; text-transform:uppercase; color:rgba(52,211,153,.5);
        }
        ...
        .contact-title {
          ...
          background:linear-gradient(135deg,#fff 40%,rgba(255,255,255,.3));
        ...
        .send-btn:hover:not(:disabled) {
          box-shadow:0 0 32px rgba(167,139,250,.55); transform:translateY(-2px);
        }
        ...
        <div style={{ position:"relative", zIndex:10, maxWidth:"1200px", margin:"0 auto", padding:"5rem 1.5rem 4rem", display:"flex", flexWrap:"wrap", alignItems:"center", gap:"3rem", minHeight:"100vh" }}>
          ...
          >
            <div style={{ position:"relative" }}>
              <div style={{ position:"absolute", inset:"-16px", borderRadius:"28px", border:"1px dashed rgba(167,139,250,.25)", animation:"contact-spin 16s linear infinite" }} />
              <div style={{ position:"absolute", inset:"-32px", borderRadius:"36px", border:"1px dashed rgba(56,189,248,.15)", animation:"contact-spin 28s linear infinite reverse" }} />
              <motion.img
              ...
              />
              <div style={{ position:"absolute", bottom:-8, left:"50%", transform:"translateX(-50%)", background:"#050810", border:"1px solid rgba(255,255,255,.1)", borderRadius:"99px", padding:"4px 14px 4px 10px", display:"flex", alignItems:"center", gap:6, zIndex:3, whiteSpace:"nowrap", fontFamily:"'JetBrains Mono',monospace", fontSize:".62rem", letterSpacing:".08em", textTransform:"uppercase", color:"rgba(255,255,255,.5)" }}>
            ...
            <div style={{ width:"100%", maxWidth:"320px", display:"flex", flexDirection:"column", gap:".75rem" }}>
              {[
                { label:"Email",    value:"ujwalshet6@gmail.com", color:"#a78bfa" },
                ...
                <motion.div key={item.label} whileHover={{ x:4 }}
                  style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:".65rem 1rem", borderRadius:"10px", background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)" }}
                >
          ...
          >
            <div style={{ background:"linear-gradient(135deg,rgba(255,255,255,.05),rgba(255,255,255,.02))", border:"1px solid rgba(255,255,255,.08)", borderRadius:"20px", overflow:"hidden", position:"relative" }}>
              ...
              {/* Terminal bar */}
              <div style={{ display:"flex", alignItems:"center", gap:".4rem", padding:".65rem 1rem", background:"rgba(0,0,0,.4)", borderBottom:"1px solid rgba(255,255,255,.06)" }}>
              ...
              <div style={{ padding:"1.75rem" }}>
                ...
                {/* ── State switcher (stays on page always) ── */}
                <AnimatePresence mode="wait">
                  ...
                  {/* SUCCESS */}
                  {status === "success" && (
                    <motion.div key="ok" initial={{ opacity:0, scale:.9 }} animate={{ opacity:1, scale:1 }} style={{ textAlign:"center", padding:"2rem 0" }}>
                      ...
                      <p style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:"1.2rem", color:"#fff", marginTop:"1rem" }}>
                        Message Transmitted! ✓
                      </p>
                      <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".65rem", color:"rgba(52,211,153,.7)", letterSpacing:".1em", marginTop:".4rem" }}>
                        I'll respond within 24 hours
                      </p>
                      {/* Inline confirmation box */}
                      <div style={{ marginTop:"1.25rem", padding:"1rem", borderRadius:"12px", background:"rgba(52,211,153,0.08)", border:"1px solid rgba(52,211,153,0.25)", fontFamily:"'JetBrains Mono',monospace", fontSize:"0.62rem", color:"rgba(52,211,153,0.8)", textAlign:"left", lineHeight:2 }}>
                        <div>$ STATUS: <span style={{ color:"#34d399" }}>DELIVERED</span></div>
                        <div>$ FROM: <span style={{ color:"#fff" }}>{formData.name || "Sender"}</span></div>
                        <div>$ CHANNEL: <span style={{ color:"#38bdf8" }}>EmailJS · Secure</span></div>
                        <div>$ RESPONSE: <span style={{ color:"#a78bfa" }}>Within 24 hours</span></div>
                      </div>
                      <motion.button
                        onClick={() => setStatus("")}
                        whileHover={{ scale:1.04 }} whileTap={{ scale:.97 }}
                        style={{ marginTop:"1.25rem", padding:".55rem 1.5rem", borderRadius:"8px", background:"rgba(167,139,250,.15)", border:"1px solid rgba(167,139,250,.35)", color:"#a78bfa", fontFamily:"'JetBrains Mono',monospace", fontSize:".65rem", letterSpacing:".1em", cursor:"pointer", width:"100%" }}
                      >
                  ...
                  {status === "error" && (
                    <motion.div key="err" initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ textAlign:"center", padding:"1.5rem 0" }}>
                      ...
                      <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:".7rem", color:"#f87171", marginTop:".75rem", letterSpacing:".08em" }}>
                        TRANSMISSION FAILED — Try again
                      </p>
                      <motion.button
                        onClick={() => setStatus("")}
                        whileHover={{ scale:1.04 }}
                        style={{ marginTop:"1rem", padding:".5rem 1.25rem", borderRadius:"8px", background:"rgba(248,113,113,.1)", border:"1px solid rgba(248,113,113,.3)", color:"#f87171", fontFamily:"'JetBrains Mono',monospace", fontSize:".62rem", cursor:"pointer", width:"100%" }}
                      >
                        Retry →
                      </motion.button>
                    </motion.div>
                  ...
                  {status === "" && (
                    <motion.div key="form" initial={{ opacity:0 }} animate={{ opacity:1 }}>
                      {/* ── IMPORTANT: div not form to prevent any browser navigation ── */}
                      <div style={{ display:"flex", flexDirection:"column", gap:"1.1rem" }}>
                        ...
                        <Field label="Your Idea" icon={FiMessageSquare} error={errors.idea} required>
                          <textarea
                            name="idea" rows={4}
                            placeholder="Describe your project or cloud infrastructure needs..."
                        ...
                        {/* Submit button — onClick instead of form submit */}
                        <motion.button
                          className="send-btn"
                          type="button"
