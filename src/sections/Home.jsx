...
function LiveTerminal() {
  ...
  useEffect(() => {
    ...
    if (charIdx < line.text.length) {
      ...
      // Line done — wait then move to next
      const t = setTimeout(() => {
  ...
  return (
    <div style={{
      ...
      boxShadow:"0 0 0 1px rgba(255,255,255,0.04), 0 32px 64px -16px rgba(0,0,0,0.9), 0 0 60px -20px rgba(167,139,250,0.2)",
      position:"relative",
    }}>
      {/* Gradient top border */}
      ...
      <div style={{
        ...
        <span style={{ marginLeft:"0.5rem", fontFamily:"'JetBrains Mono',monospace", fontSize:"0.6rem", color:"rgba(255,255,255,0.3)", letterSpacing:"0.06em" }}>
          ujwal@aws-cloud — bash
        </span>
        {/* Blinking status */}
      ...
      >
        {lines.map((line, i) => (
          <div key={i} style={{ display:"flex", gap:"0.5rem", flexWrap:"wrap" }}>
            <span style={{ color: line.prompt === "$" ? "rgba(52,211,153,0.7)" : "rgba(255,255,255,0.25)", flexShrink:0 }}>
              {line.prompt}
            </span>
...
function MouseSpotlight() {
  ...
  return (
    <div style={{
      ...
      background:`radial-gradient(600px circle at ${pos.x}px ${pos.y}px, rgba(167,139,250,0.06) 0%, transparent 70%)`,
  ...
  return (
    <>
      <style>{`
        ...
        /* Glitch */
        .glitch-wrap { color:#fff; }
        .glitch-wrap::before,.glitch-wrap::after {
          content:attr(data-text); position:absolute; top:0; left:0; width:100%; background:transparent;
        }
        .glitch-wrap::before {
          color:#38bdf8; clip-path:polygon(0 30%,100% 30%,100% 50%,0 50%);
          animation:glitch-top 4s infinite linear; text-shadow:-2px 0 #38bdf8;
        }
        .glitch-wrap::after {
          color:#a78bfa; clip-path:polygon(0 60%,100% 60%,100% 80%,0 80%);
          animation:glitch-bot 3.5s infinite linear; text-shadow:2px 0 #a78bfa;
        }
        @keyframes glitch-top {
          0%,90%,100%{transform:translateX(0)} 92%{transform:translateX(-3px)} 96%{transform:translateX(3px)}
        }
        @keyframes glitch-bot {
          0%,85%,100%{transform:translateX(0)} 87%{transform:translateX(3px)} 91%{transform:translateX(-3px)}
        }
